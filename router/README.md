# Zunzun router — the routing OS layer (R5)

One OpenAI-compatible endpoint in front of the Zunzun engine. Classifies each
request, dispatches to a backend *profile*, lazy-spawns it via `zun serve`,
...
- `zunzun-gpu` — Zunzun with the HIP iGPU tier (`COLI_HIP=1 CUDA_DENSE=1 CUDA_EXPERT_GB=N`), `device: igpu`.
- `zunzun-cpu` — pure CPU streaming, `device: cpu`.
...
## Routing
- `model: "zunzun-gpu"` / `"zunzun-cpu"` → that profile explicitly.
- `model: "auto"` → `default` (zunzun-gpu), unless the prompt is very long or
...
proxy). It becomes useful once `zun serve` has a runnable model (a model dir
