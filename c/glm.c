...
static int g_cuda_devices[COLI_GPU_MAX_DEVICES], g_cuda_ndev, g_cuda_rr;
static int64_t g_cuda_dense_projected[COLI_GPU_MAX_DEVICES];
static void qt_cuda_reset(QT *t){
    if(t->cuda){ coli_gpu_tensor_free(t->cuda); t->cuda=NULL; }
    t->cuda_failed=0;
}
static int qt_cuda_upload(QT *t){
    const void *weights = t->fmt==0 ? (const void*)t->qf
                        : t->fmt==1 ? (const void*)t->q8 : (const void*)t->q4;
    return coli_gpu_tensor_upload(&t->cuda,weights,t->s,t->fmt,t->I,t->O,t->cuda_device);
}
...
static int parse_cuda_devices(const char *list, int *out){
    ...
    while(*p){
        ...
        if(end==p||v<0||v>INT_MAX||n>=COLI_GPU_MAX_DEVICES) return 0;
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot){
    ...
    if(g_cuda_enabled && w->cuda_eligible && !w->cuda_failed && !omp_in_parallel()
       && sched_place(SCHED_MATMUL,S,qt_bytes(w),1,1)==SCHED_GPU){  /* R3: size-aware placement gate */
        const void *weights = w->fmt==0 ? (const void*)w->qf
                            : w->fmt==1 ? (const void*)w->q8 : (const void*)w->q4;
        if(coli_gpu_matmul(&w->cuda,y,x,weights,w->s,w->fmt,S,w->I,w->O,w->cuda_device)) return;
        w->cuda_failed=1;
...
static int expert_load(Model *m, int layer, int eid, ESlot *s, int fatal){
    ...
    for(int k=0;k<3;k++){
        ...
        mp_qs_name(&m->mp,nm[k],qn,sizeof(qn)); tq[k]=st_find(&m->S,qn);   /* R0: via provider */
...
#ifdef COLI_GPU
/* generic host-memory ops (munlock + free of the RAM copy once the expert
 * computes from VRAM): nothing CUDA-specific, HIP tier uses them too */
    ...
    for(int base=0;base<nu;base+=64){
        ...
        if(base+64<nu){
            ...
            for(int j=0;j<nb2;j++){ int eid=uniq[base+64+j];
                if(!ec_resident(m,layer,eid)) expert_prefetch(m,layer,eid);   /* R1 */
            }
        }
#ifdef COLI_CUDA
...
static void repin_pass_limit(Model *m,int limit){
    ...
    for(int b=0;b<nb;b++){
        ...
        int64_t old_gpu=gpu ? (int64_t)coli_gpu_tensor_bytes(s->g.cuda)
                             +(int64_t)coli_gpu_tensor_bytes(s->u.cuda)
                             +(int64_t)coli_gpu_tensor_bytes(s->d.cuda) : 0;
#endif
        double t0=now_s();
        ...
        if(gpu){                                  /* refresh the same VRAM slot now, not lazily */
            if(qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)){
                int64_t now_gpu=(int64_t)coli_gpu_tensor_bytes(s->g.cuda)
                               +(int64_t)coli_gpu_tensor_bytes(s->u.cuda)
                               +(int64_t)coli_gpu_tensor_bytes(s->d.cuda);
...
static void kv_disk_truncate(Model *m, int nrec){
    ...
    int32_t nw[2]={nrec,k->disk_wm}; fseek(f,8+6*4,SEEK_SET); fwrite(nw,4,2,f);
    fflush(f); fclose(f);
}
...
static void kv_disk_append(Model *m, const int *hist, int len, int complete){
    ...
    int32_t nw[2]={len,k->disk_wm}; fseek(f,8+6*4,SEEK_SET); fwrite(nw,4,2,f);
...
static void mux_done(Model *m, ServeCtx *sc, ServeReq *r){
    ...
    kv_disk_append(m,sc->hist,sc->len,!r->length_limited);  /* completo = fermato da EOS, non dal limite */
    r->active=0;
}
...
static int mux_submit(Model *m, Tok *T, ServeCtx *ctx, ServeReq *req, int nctx,
    ...
    if(!strncmp(line,"CANCEL ",7)){
        ...
        for(int i=0;i<nctx;i++) if(req[i].active && req[i].id==id){
            ...
            kv_disk_append(m,ctx[i].hist,ctx[i].len,0);  /* CANCEL = mezzo turno: watermark fermo */
...
static void run_serve(Model *m, const char *snap){
     ...
     * 'zun' matches the sentinels with endswith() and a "^STAT ..." regex,
...
static int pin_rec_cmp(const void *a,const void *b){
    const PRec *x=a,*y=b; return x->c<y->c?1:x->c>y->c?-1:0;
}
...
static int pin_load_recs(Model *m, PRec *r, int n, double gb, const char *src){
    ...
    if(npin>n) npin=n;
    if(npin<1) return 0;                       /* r resta al chiamante */
    ...
    int placed_n[COLI_GPU_MAX_DEVICES]={0}, gpu_prefix=0;
    ...
    if(g_cuda_enabled&&(g_cuda_expert_gb>0||g_cuda_expert_auto)) for(int i=0;i<g_cuda_ndev;i++){
        ...
        if(coli_gpu_mem_info(g_cuda_devices[i],&free_b,&total_b)){
    ...
    if(g_cuda_enabled && budget>0){
        ...
        for(int a=0;a<gpu_limit && m->gpu_expert_bytes<budget;a++){
            ...
            { ESlot *s=&m->pin[li][slot_of[a]];
                ...
                int tried[COLI_GPU_MAX_DEVICES]={0}, placed=0;
                for(int attempt=0;attempt<g_cuda_ndev && !placed;attempt++){
                    ...
                    if(qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)){
                        int64_t actual=(int64_t)coli_gpu_tensor_bytes(s->g.cuda)
                                      +(int64_t)coli_gpu_tensor_bytes(s->u.cuda)
                                      +(int64_t)coli_gpu_tensor_bytes(s->d.cuda);
    ...
    fprintf(stderr,"[PIN] placement: %d VRAM + %d RAM expert (%.1f GB warm) in %.0fs da %s\n",
        m->gpu_expert_count,npin-m->gpu_expert_count,(npin-m->gpu_expert_count)*eb/1e9,now_s()-t0,src);
    pin_wire(m);                                   /* inchioda in RAM (no compressione) / wire in RAM (no compression) */
    free(cnt_l); free(slot_of); free(next); return npin;
}

/* wrapper: legge un file STATS "layer eid count", dedupe + filtro validita'
 * (sparse/MTP), PIN_FILL opzionale, e delega a pin_load_recs.
 * EN: file-parsing front half of the pin (dedupe, PIN_FILL), then load. */
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    while(n<cap && fscanf(f,"%d %d %u",&l,&e,&cnt)==3){
        ...
        if(ok&&!seen[key]){ r[n++]=(PRec){l,e,cnt}; seen[key]=1; }
    }
    fclose(f);
    ...
    if(fill) for(int li=0;li<=c->n_layers;li++){
        ...
        if(sparse) for(int ei=0;ei<c->n_experts;ei++) if(n<cap&&!seen[(int64_t)li*c->n_experts+ei])
            r[n++]=(PRec){li,ei,0};
    }
    free(seen);
...
int main(int argc, char **argv){
...
#ifdef COLI_GPU
    if((getenv("COLI_CUDA") && atoi(getenv("COLI_CUDA"))) || (getenv("COLI_HIP") && atoi(getenv("COLI_HIP")))){
        ...
        g_cuda_enabled=coli_gpu_init(g_cuda_devices,g_cuda_ndev);
    ...
    if(g_draft<0){
        ...
        g_draft = (m.has_mtp && (!g_cuda_enabled || cuda_mtp)) ? 4 : 0;
#else
        /* d4: sweep Task-10 2026-07-17 (chain-norm fix) — 3.43 tok/fw, best tok/s
         * after d2. HIP (COLI_HIP) takes this path too: dense-on-GPU, MTP head on
         * CPU, no kernel-family divergence measured on this box. */
        g_draft = m.has_mtp ? 4 : 0;
#endif
    }
